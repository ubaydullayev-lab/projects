...
import {
  FaInstagram, FaTelegram, FaLinkedin, FaYoutube,
  FaTiktok, FaXTwitter
    ...
    <section id="contact" style={{ padding: '80px 20px', background: 'linear-gradient(145deg, #111, #1a1a1a)' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', color: 'white' }}>
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <Title level={2} style={{ textAlign: 'center', color: '#00f5ff' }}>{t('contact.title')}</Title>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '40px', justifyContent: 'center', marginTop: '50px' }}>
            ...
            <div style={{ flex: 1, minWidth: '300px', background: '#222', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)' }}>
              <Form layout="vertical" onFinish={onFinish} form={form}>
                <Form.Item name="name" rules={[{ required: true, message: t('contact.name') + '!' }]}
                  label={<span style={{ color: '#00f5ff' }}>{t('contact.name')}</span>}>
                  <Input placeholder={t('contact.name')} size="large" />
                </Form.Item>

                <Form.Item name="email" rules={[{ required: true, type: 'email', message: t('contact.email') + '!' }]}
                  label={<span style={{ color: '#00f5ff' }}>{t('contact.email')}</span>}>
                  <Input placeholder={t('contact.email_info')} size="large" />
                </Form.Item>

                <Form.Item name="message" rules={[{ required: true, message: t('contact.message') + '!' }]}
                  label={<span style={{ color: '#00f5ff' }}>{t('contact.message')}</span>}>
                  <Input.TextArea rows={5} placeholder={t('contact.message')} />
                </Form.Item>
                ...
                <Form.Item>
                  <Button
                    ...
                    htmlType="submit"
                    size="large"
                    style={{ background: '#00f5ff', borderColor: '#00f5ff', color: 'black' }}
                    disabled={sending}
                  >
                ...
                {message && (
                  <p style={{ color: message.startsWith('✅') ? 'lime' : 'red', fontWeight: 'bold' }}>{message}</p>
                )}
              </Form>
            </div>
            ...
            <div style={{ flex: 1, minWidth: '250px', background: '#222', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)' }}>
              ...
              <div style={{ marginTop: '30px' }}>
                ...
                <div style={{ display: 'flex', gap: '90px', marginTop: '50px', flexWrap: 'wrap' }}>
                  ...
                  <motion.a href="https://youtube.com/@youtube_lochinbek?si=pzx2Y4MRr4Luiufi" target="_blank" rel="noreferrer"
                  ...
                  <motion.a href="https://x.com/Lochin008?s=09" target="_blank" rel="noreferrer"
                    whileHover={{ scale: 1.2, rotate: -3 }} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 300 }}>
